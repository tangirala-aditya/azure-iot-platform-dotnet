...
function createDiagnosticSettings([string]$resourceGroupName, [string]$subscriptionId, [string]$iotHubName, [string]$loganalyticsName) {
    ...
    Set-AzDiagnosticSetting -Name $diagnosticSetting -ResourceId $resourceId -Category Connections, DeviceTelemetry, C2DCommands, DeviceIdentityOperations, FileUploadOperations, Routes, D2CTwinOperations, C2DTwinOperations, TwinQueries, JobsOperations, DirectMethods, DistributedTracing, Configurations, DeviceStreams, AllMetrics -MetricCategory AllMetrics -Enabled $true -WorkspaceId $workSpaceID   
}
