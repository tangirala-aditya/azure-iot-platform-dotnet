                    ...
                    foreach (var deviceGroupMapping in deviceMappings)
                    {
                        ...
                        if (existingDeviceGroupMappings != null && existingDeviceGroupMappings.Any())
                        {
                            ...
                            if (newDeviceIds != null && newDeviceIds.Count() > 0)
                            {
                                ...
                                {
                                    QueueClient queueClient = new QueueClient(this.appConfig.Global.StorageAccountConnectionString, "devicesforMapping", new QueueClientOptions() { MessageEncoding = QueueMessageEncoding.Base64 });
                                    await queueClient.CreateIfNotExistsAsync();

                                    foreach (var data in batchedData)
                                    {
                                        // Insert into Queue
                                        DeviceGroupMappingRequest deploymentRequest = new DeviceGroupMappingRequest
                                        {
                                            TenantId = tenantId,
