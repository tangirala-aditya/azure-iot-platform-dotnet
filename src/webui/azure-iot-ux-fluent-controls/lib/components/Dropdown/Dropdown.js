...
const React = require("react"),
    ...
    css = classNames.bind(require("./Dropdown.module.scss"));
...
(function (DropdownAlignment) {
    ...
    (DropdownAlignment =
        exports.DropdownAlignment || (exports.DropdownAlignment = {}))
);
...
class Dropdown extends React.Component {
    constructor(props) {
        ...
        this.dropdownRef = (element) =>
            (this.dropdown = element ? element : this.dropdown);
        this.containerRef = (element) =>
            (this.container = element ? element : this.container);
        this.fixedRef = (element) =>
            (this.fixedContainer = element ? element : this.fixedContainer);
        this.onChange = (event) => {
        ...
        this.handleOuterEvent = (event) => {
            if (
                event.target === this.fixedContainer ||
            ...
            if (
                this.props.onOuterEvent &&
                !this.interactsWithDropdown(event, 9)
            ) {
    ...
    componentDidUpdate(oldProps, oldState) {
        ...
        if (this.props.visible) {
            ...
            if (this.props.onMouseEnter || this.props.onMouseLeave) {
                this.fixedContainer.className = css(
    ...
    getDropdownOffset() {
        const container = this.container.getBoundingClientRect(),
            dropdown = this.dropdown.getBoundingClientRect();
        return {
    ...
    updateEventHandlers() {
        if (this.props.visible && !this.eventsConnected) {
            ...
            if (this.props.outerEvents) {
                this.props.outerEvents.forEach((event) =>
                    window.addEventListener(event, this.handleOuterEvent)
                );
            }
            ...
            if (this.props.outerEvents) {
                this.props.outerEvents.forEach((event) =>
                    window.removeEventListener(event, this.handleOuterEvent)
                );
            }
    ...
    repositionDropdown() {
        ...
        if (
            this.fixedContainer &&
            this.fixedContainer.parentElement !==
            ...
            if (
                ...
                if (typeof this.state.positionIndex === "number") {
                    if (this.positionReset) {
                        ...
                        this.props.positionClassNames.length
                    ) {
                        this.setState({
        ...
        const container = this.container.getBoundingClientRect(),
            positioned = this.dropdown.parentElement === this.fixedContainer;
        if (!positioned) {
            this.dropdownOffset = this.getDropdownOffset();
        ...
        if (!compareClientRect(container, this.previousPosition)) {
            this.fixedContainer.style.left = `${
                container.left + this.dropdownOffset.left
            }px`;
            this.fixedContainer.style.top = `${
                container.top + this.dropdownOffset.top
            }px`;
        ...
        if (
            ...
            this.fixedContainer.style.top = `${
                container.top - this.dropdownOffset.height
            }px`;
            this.dropdownDirection = "top";
        } else if (this.dropdownDirection === "top") {
            if (
                container.top + container.height + dropdownRect.height <
                window.innerHeight
            ) {
