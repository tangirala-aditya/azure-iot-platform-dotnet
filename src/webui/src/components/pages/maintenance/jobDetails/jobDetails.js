...
export class JobDetails extends Component {
    ...
    handleNewProps(nextProps) {
        ...
        if (
            ...
            const selectedJob = nextProps.jobs.filter(
    ...
    getJobStatus(jobId) {
        ...
        this.subscription = IoTHubManagerService.getJobStatus(jobId).subscribe(
            (jobStatus) =>
                this.setState({ jobStatus, jobStatusIsPending: false }),
            (jobStatusError) =>
                this.setState({ jobStatusError, jobStatusIsPending: false })
        );
    }
    ...
    render() {
        const {
            ...
            jobStatusGridProps = {
                domLayout: "autoHeight",
                rowData: this.state.jobStatusIsPending
                    ? undefined
                ...
                onRowClicked: ({ data: { devices } }) => {
                    this.setState({
                        selectedDevices: devices.map(
                            ({ deviceId }) => deviceEntities[deviceId]
                        ),
                    });
