...
export class RuleEditor extends LinkedComponent {
    ...
    apply = (event) => {
        ...
        const { insertRules, modifyRules, logEvent, t } = this.props,
            requestProps = { ...this.state.formData },
            { devicesAffected, newEmail } = this.state;
        if (requestProps.calculation === ruleCalculations[1]) {
            requestProps.timePeriod = "";
        }
        if (this.state.formData.actionEnabled === false) {
            requestProps.actions = [];
        }
        if (this.formIsValid()) {
                ...
                this.subscription = IoTHubManagerService.getDevices(
                    ...
                    .subscribe(
                        (groupDevicesAndModules) => {
                            let groupDevices = groupDevicesAndModules[0],
                                modules = groupDevicesAndModules[1];
                            this.setState({
                                fieldQueryPending: false,
                                fieldOptions: this.getConditionFields(
                                    groupDevices,
                                    modules
                                ),
    ...
    onFieldChange = (
        ...
        let eventProperties = { FieldChosen: value, ConditionNumber: index };
        this.props.logEvent(
            toDiagnosticsModel("Rule_FieldClick", eventProperties)
        );
        this.formControlChange();
    };
    ...
    onOperatorChange = (
        ...
        let eventProperties = { OperatorChosen: value, ConditionNumber: index };
        this.props.logEvent(
            toDiagnosticsModel("Rule_OperatorClick", eventProperties)
        );
        this.formControlChange();
    };
    ...
    render() {
        const { t, deviceGroups = [] } = this.props,
            ...
            calculationOptions = ruleCalculations.map((value) => ({
                label: t(
                    `rules.flyouts.ruleEditor.calculationOptions.${value.toLowerCase()}`
                ),
                value,
            })),
        ...
        this.deviceGroupLink = this.formDataLink
            ...
            .map(({ value }) => value)
            .withValidator(requiredValidator);
        this.calculationLink = this.formDataLink
            .forkTo("calculation")
            .map(({ value }) => value)
            .withValidator(requiredValidator);
        this.timePeriodLink = this.formDataLink
            .forkTo("timePeriod")
            .map(({ value }) => value)
            .check(
                (timePeriod) =>
                    this.calculationLink.value === ruleCalculations[0]
                        ? Validator.notEmpty(timePeriod)
                        : true,
        ...
        const conditionLinks = this.conditionsLink.getLinkedChildren(
                (conditionLink) => {
                    const fieldLink = conditionLink
                            ...
                            .map(({ value }) => value)
                            .withValidator(requiredValidator),
                        operatorLink = conditionLink
                            .forkTo("operator")
                            .map(({ value }) => value)
                            .withValidator(requiredValidator),
                        valueLink = conditionLink
                            .forkTo("value")
                            .check(Validator.notEmpty, () =>
                                this.props.t(
                                    "rules.flyouts.ruleEditor.validation.required"
                                )
                            )
                            .check(
                                (val) => !isNaN(val),
            ...
            actionLinks = this.actionsLink.getLinkedChildren((actionLink) => {
                const parametersLink = actionLink.forkTo("parameters"),
                    ...
                    notesLink = parametersLink.forkTo("notes"),
                    subjectLink = parametersLink.forkTo("subject"),
                    error = formData.actionEnabled
                        ? recipientsLink.error
                        : false;
                return { recipientsLink, notesLink, subjectLink, error };
            }),
            conditionsHaveErrors = conditionLinks.some(({ error }) => error),
            actionsHaveErrors = actionLinks.some(({ error }) => error),
            completedSuccessfully = changesApplied && !error,
            operatorOptions = ruleOperators.map((value) => ({
                label: t(
                    `rules.flyouts.ruleEditor.condition.operatorOptions.${value}`
                ),
                value,
            }));
                    ...
                    <Section.Content>
                        <FormGroup>
                            ...
                            <FormControl
                                ...
                                className="long"
                                placeholder={t(
                                    "rules.flyouts.ruleEditor.namePlaceholder"
                                )}
                        ...
                        <FormGroup>
                            <FormLabel>
                                {t("rules.flyouts.ruleEditor.description")}
                            </FormLabel>
                            <FormControl
                                type="textarea"
                                placeholder={t(
                        ...
                        <FormGroup>
                            ...
                            <FormControl
                                type="select"
                                ariaLabel={t(
                                    "rules.flyouts.ruleEditor.deviceGroup"
                                )}
                                className="long"
                        ...
                        <FormGroup>
                            ...
                            <FormControl
                                type="select"
                                ariaLabel={t(
                                    "rules.flyouts.ruleEditor.calculation"
                                )}
                                className="long"
                                placeholder={t(
                                    "rules.flyouts.ruleEditor.calculationPlaceholder"
                                )}
                        ...
                        {this.calculationLink.value === ruleCalculations[0] && (
                            <FormGroup>
                                ...
                                <FormControl
                                    type="select"
                                    ariaLabel={t(
                                        "rules.flyouts.ruleEditor.timePeriod"
                                    )}
                                    className="short"
                                    onChange={this.formControlChange}
                ...
                {!fieldQueryPending && (
                    <div>
                        {conditionLinks.map((condition, idx) => (
                            <Section.Container
                                key={formData.conditions[idx].key}
                            >
                                <Section.Header>
                                    {t(
                                        "rules.flyouts.ruleEditor.condition.condition"
                                    )}{" "}
                                    {idx + 1}
                                </Section.Header>
                                <Section.Content>
                                    {idx === 0 && fieldOptions.length === 0 && (
                                        <SectionDesc>
                                            <Trans
                                                i18nKey={
                                                    "rules.flyouts.ruleEditor.condition.description"
                                                }
                                            >
                                    ...
                                    {conditionLinks.length > 1 && (
                                        ...
                                        >
                                            {t(
                                                "rules.flyouts.ruleEditor.delete"
                                            )}
                                        </Btn>
                                    )}
                                </Section.Content>
                            ...
                            <Section.Container collapsable={false}>
                                ...
                                <div className="email-toggle-container">
                                    <Toggle
                                        ...
                                        onLabel={t(
                                            "rules.flyouts.ruleEditor.actions.on"
                                        )}
                                        offLabel={t(
                                            ...
                                            >
                                                ...
                                                <FormGroup>
                                                    <FormControl
                                                        type="text"
                                                        className="long"
                                                        onKeyPress={this.onAddEmail(
                                                            action.recipientsLink
                                                        )}
                            ...
                            <Section.Container>
                                ...
                                <Section.Content>
                                    <FormGroup>
                                        <Toggle
                                            name={t(
                                                "rules.flyouts.ruleEditor.ruleStatus"
                                            )}
                                            attr={{
                                                button: {
                                                    "aria-label": t(
                                            ...
                                            onLabel={t(
                                                "rules.flyouts.ruleEditor.ruleEnabled"
                                            )}
                                            offLabel={t(
                                                "rules.flyouts.ruleEditor.ruleDisabled"
                                            )}
                                        />
                                    </FormGroup>
                                </Section.Content>
