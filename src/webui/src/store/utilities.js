...
export const createAction = (type, staticPayload) => {
    const creator = (payload, meta = {}) => ({ type, payload, ...meta }),
        useStaticPayload = typeof staticPayload !== "undefined";
...
export function createEpicCase(params) {
    if (
        (!params.type && !params.rawType) ||
        (!params.epic && !params.rawEpic)
    ) {
        throw new Error(
    ...
    const type = params.rawType || `EPIC_${params.type}`,
        action = createAction(type, params.staticPayload),
        epic = chooseEpicParam(type, params.epic, params.rawEpic);
...
export function createReducerScenario(cases = {}) {
    ...
    const { actionTypes, actions, reducers } = Object.keys(cases).reduce(
            (acc, caseName) => {
                const { type, action, reducer } = createReducerCase(
                    cases[caseName]
                );
                return {
                    actionTypes: { ...acc.actionTypes, [caseName]: type },
        ...
        actionReducers = Object.keys(reducers).reduce((acc, actionName) => {
            const actionType = actionTypes[actionName],
                matchTypes = Array.isArray(actionType)
                    ? actionType
                    : [actionType],
                reducer = reducers[actionName],
                reducerMapping = matchTypes.reduce(
