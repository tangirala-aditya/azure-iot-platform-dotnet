...
export class ExampleGrid extends Component {
        ...
        this.contextBtns = (
            <ComponentArray>
                <Btn
                    ...
                    onClick={this.clickContextBtn("btn1")}
                >
                    {props.t("walkthrough.pageWithGrid.grid.btn1")}
                </Btn>
                <Btn svg={svgs.trash} onClick={this.clickContextBtn("btn2")}>
    ...
    onHardSelectChange = (selectedObjs) => {
        ...
        if (isFunc(onContextMenuChange)) {
            onContextMenuChange(
                selectedObjs.length > 0 ? this.contextBtns : null
            );
        }
    ...
    render() {
        ...
        return <PcsGrid key="example-grid-key" {...gridProps} />;
    }
}
// </grid>
